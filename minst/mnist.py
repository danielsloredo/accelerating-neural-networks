...
if __name__ == '__main__':
    ...
    trainloader = torch.utils.data.DataLoader(dataset = dataset, batch_size = 200, shuffle = True)
    
    trainloader2 = torch.utils.data.DataLoader(dataset = dataset, batch_size = 60000)
    for img,_ in trainloader2:
        testTensor = img.reshape(-1, 28*28).to(device)
    ...
    loss_all = np.zeros((3,6*num_epoch+1))
    ...
    if not os.path.exists("net_init_mnist.pt"):
        net_init.apply(weights_init_sparse)
        torch.save(net_init.state_dict(), "net_init_mnist.pt")
    else:
        net_init.load_state_dict(torch.load("net_init_mnist.pt"))
    ...
    np.savez("{}_{}_ae_mnist".format(lam_max,step)+'_final.npz', 
             loss_all = loss_all, weight_list_init = weight_list_init, 
             weight_list_final = weight_list_final, final_loss = final_loss)